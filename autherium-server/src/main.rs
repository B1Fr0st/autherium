...
async fn create_license(req:web::Json<CreateRequest>,state: web::Data<State>) -> Result<impl Responder> {
    if req.key != "super_secret_key"{ //fixme
        return Err(
        ...
        if regen_counter > 100 {
            return Err(
                error::InternalError::from_response(
                    ...
                    HttpResponse::InternalServerError().json(ErrorResponse::new("Failed to generate a unique license key."))
                ).into()
            );
        }
        s = rand::rng()
            ...
            .collect::<String>()
            .to_uppercase();
        assert!(regex.is_match(&s), "Generated license key does not match regex");
...
async fn auth(req: web::Json<AuthRequest>, state: web::Data<State>) -> Result<impl Responder> {
    ...
    if state.banned_hwids.lock().unwrap().iter().find(|entry| **entry == req.hwid).is_some(){
        return Ok(HttpResponse::Unauthorized().json(ErrorResponse::new("Your HWID is banned.")));
    }

    let regex = regex::Regex::new(r"^[A-Z0-9]{16}").unwrap();

    if regex.is_match(&req.license) == false {
        return Err(
            error::InternalError::from_response(
                "Not a valid license.",
    ...
    if let Some(license) = state.licenses.lock().unwrap().iter_mut().find(|entry|{entry.key == req.license}) {
        if !license.used{
            license.start();
        }
        let time_remaining = (license.start + license.duration) as i64 - std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs() as i64;
        if time_remaining <= 0 {
...
async fn main() -> std::io::Result<()> {
    let state = web::Data::new(State{
        ...
        banned_hwids: Mutex::new(Vec::new())
    });
    ...
    HttpServer::new(move || {
        ...
        let auth_json_config = web::JsonConfig::default()
            .limit(4096)
            .error_handler(|err, _| {
                error::InternalError::from_response(err, HttpResponse::BadRequest().json(ErrorResponse::new("The auth payload was malformed.")).into()).into()
            });

        
        App::new()
            .service(
                web::scope("/api/v1").app_data(auth_json_config).app_data(state.clone())
                .service(auth)
